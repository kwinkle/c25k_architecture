...
if generator_type == ACTION:
	...
	for index, row in relevant_actions.iterrows():
		...
		if user_id in blacklisted:
			if not session_number in blacklisted[user_id]:
				...
				with open('new_action_instances_noface.csv', 'a') as logfile:
	...
	actionclass_instances_df = pd.read_csv('new_action_instances_noface.csv', names=['action_id','actionclass',\
		...
		'relative_speed_average', 'relative_speed_best', 'normalised_time_since_last_action', 'relative_heart_rate', 'session_mood',\
		'reward'])
	...
	for index, row in relevant_actions.iterrows():
		...
		if user_id in blacklisted:
			if not session_number in blacklisted[user_id]:
				...
				with open('new_style_instances_noface.csv', 'a') as logfile:
					logfile.write(action_record + "\n")
					logfile.flush()
	...
	style_instances_df = pd.read_csv('new_style_instances_noface.csv', names=['action_id','style',\
		'self_attitude', 'expert_attitude', 'extraversion','agreeableness', 'conscientiousness', 'emotional_stability', 'openness_experience', 'activity_level',\
		'programme_state', 'normalised_session_time', 'normalised_programme_time', 'programme_action_progress', 'programme_action_duration',\
		'relative_speed_average', 'relative_speed_best', 'normalised_time_since_last_action', 'relative_heart_rate', 'session_mood',\
		'reward'])
